...
def Click(url):
    ...
    driver = webdriver.Chrome(executable_path='C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe',options=option)
    driver.get(url)
    driver.maximize_window()
    driver.implicitly_wait(10)  # 最多等待时间 页面加载时间
    #Justone(driver,num,listnum)
    #Nornoal(driver,numone,numtwo,listnum)
    #Choose(driver,islist,listnum)
    #Manychoice(driver,i,listnum)
    #Cmanychoice(driver,islist,listnum)
    # 1
    Nornoal(driver, 1,2,AA)
    # ，
    Nornoal(driver, 3,4, BB)
    # 3
    Nornoal(driver, 1, 2, CC)
    # 4
    Nornoal(driver, 1, 3, DD)
    # 5
    Nornoal(driver, 1, 3, EE)
    # 6、
    Choose(driver, [1, 3, 5], FF)
    # 7
    Nornoal(driver, 2, 3, GG)
    # 8
    Justone(driver, 1, HH)
    # 9
    Manychoice(driver, 2, II)
    # 10
    Cmanychoice(driver, [1, 4], JJ)
    # 11
    Cmanychoice(driver, [1,2, 5], KK)
    # 12
    Nornoal(driver, 1, 2, LL)
    # 13
    Nornoal(driver, 1, 3, MM)
    # 14
    Nornoal(driver, 1, 2, NN)
...
if __name__ == '__main__':
    url = "https://www.wjx.cn/m/72978818.aspx?from=singlemessage"
    for i in range(40):#输入数字
        try:
            t1 = time.time()
            Click(url)
            t2 = time.time()
