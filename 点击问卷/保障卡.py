...
def Click(url):
    ...
    # 4
    Nornoal(driver, 2, 3, DD)
    # 5
    Nornoal(driver, 4,5 , EE)
    # 6、
    Nornoal(driver, 1,2, FF)
    # 7
    Nornoal(driver, 2, 3, GG)
    # 8
    Manychoice(driver, 4, HH)
    # 9
    Nornoal(driver, 1, 2, II)
    # 10
    Nornoal(driver, 1, 2, JJ)
    # 11
    Justone(driver, 1, KK)
    # 12
    Justone(driver, 1, LL)
...
if __name__ == '__main__':
    url = "https://www.wjx.cn/m/72163644.aspx?from=timeline"
    for i in range(50):#输入数字
        try:
            t1 = time.time()
            Click(url)
            t2 = time.time()
